#!/usr/bin/env python
"""Author ``ip3r/resources/parameters.json`` behind a provenance gate.
...
from ip3r.config import RESOURCE_DIR  # noqa: E402
...
from reference_table import REFERENCES  # noqa: E402
    ...
    for entry in entries:
        ...
        if citation in SENTINELS:
            ...
            problems.append(f"{key}: citation {citation!r} is not in "
                            f"reference_table.py — add the reference first")
...
def main() -> int:
    ...
    if args.check:
        return 0
    (RESOURCE_DIR / "references.json").write_text(json.dumps(
        {"schema": 1, "references": REFERENCES}, indent=1))
