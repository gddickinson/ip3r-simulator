...
backbone-only regions of large cryo-EM models), and produces a demonstrably
untwisted ribbon.

Ported from the PIEZO1 simulator (``piezo1_simulation``); it carries no
family constants.
"""
...
def assign_secondary_structure(ca: np.ndarray, smooth: int = 2) -> np.ndarray:
    ...
    large cryo-EM models are backbone-only, where a hydrogen-bond method
    ...
    # a PIEZO1 helix (8YEZ) gives d2 5.52, d3 5.34, d4 6.29, torsion +51 (ported).
    helix = (
