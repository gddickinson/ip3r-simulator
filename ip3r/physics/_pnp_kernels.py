...
Ported unchanged from the PIEZO1 simulator (``piezo1/physics/_pnp_kernels.py``);
the "Round N" notes below are that project's history, kept because they say
...
that go into it, moved here when PIEZO1 Round 81 added fixed charge and
...
def _nernst_planck(z, area, potential, valence, diffusivity, thermal,
    ...
    **The drift term's sign was inverted here until PIEZO1 Round 81.** The two
...
def _ohmic_potential(z, face_conductance, v_left, v_right):
    ...
    condition under which every PIEZO1 number before its Round 81 was computed. As soon as either fails, the closure is
