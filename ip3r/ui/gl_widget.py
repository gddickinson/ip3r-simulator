...
class ViewportWidget(QOpenGLWidget):
    ...
    def paintGL(self) -> None:
        ...
        fbo.use()
        # Size the viewport from the framebuffer actually bound, not from the
        # last resizeGL: QWidget.grab() renders into an FBO at a different
        # pixel ratio, and a stale viewport then draws a cropped quarter of
        # the scene (found by the screenshot smoke test).
        if tuple(self.scene.viewport[2:]) != tuple(fbo.size):
            self.scene.resize(*fbo.size)
        self.scene.render()
