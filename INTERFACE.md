...
| `cli_ryr.py` | the RyR1 commands, `register(sub)`: `mutants`, `ryr-gating`, `sparks [--scan] [--cleft [--fit]]`, `spark-termination [--scan fit\|ki\|rate\|use\|ratio] [--bell] [--fitted]`, `spark-mg [--scan] [--reading R] [--spontaneous] [--two-site]`, `ec [--scan] [--reading R] [--only S] [--trials N] [--two-site] [--use] [--depletion [--pool-scan] [--large]]` |
...
| `spark_termination.py` | what ends a cleft spark: `Termination` (sparks, median duration, `unterminated`, inactivated at end, open fraction), `measure(sp)`, `refit` (Stern vs fitted to Murayama 25/37 °C), `ki_scan`, `rate_scan(base)` (Ki fixed, rates scaled), `use_scan` (the use gate's speed at the registered recovery ratio, Ca²⁺ gate refitted at each), `ratio_scan` (its recovery/inactivation ratio ρ, the round's load-bearing constant), `no_inactivation` (neither gate: the induction-decay control) |
