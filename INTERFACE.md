...
| `cli_ryr.py` | the RyR1 commands, `register(sub)`: `mutants`, `ryr-gating`, `sparks [--scan] [--cleft [--fit]]`, `spark-termination [--scan fit\|ki\|rate] [--fitted]`, `spark-mg [--scan] [--reading R] [--spontaneous]`, `ec [--scan] [--reading R] [--only S] [--trials N] [--depletion [--pool-scan] [--large]]` |
...
| `ec_release.py` | release under voltage clamp: `ensemble(v, scale=)` → `Ensemble`, `summarise` → `Summary` (peak, plateau, after repolarisation, flux peak/plateau), `configurations(reading)` (Stern / fitted / fitted + Mg²⁺ at the activation site / both sites), `with_gating`, `events`, `event_stats` → `EventStats` (single C events at `ec.spark_voltage`), `voltages` |
