...
| `cli_perm.py` | `register(sub)`: `selectivity [PDB]` (Vais 2010's P_Cl:P_K, P_Ca:P_K, i_Ca on every wall reading) |
...
| `permeation.py` | 1-D drift-diffusion (ported from PIEZO1): `IonSpecies`, `potassium_species` (symmetric KCl; sweep overrides), `solve_pnp(z, r_free, fixed_charge=)` → `PermeationResult` (ohmic closure uncharged, local electroneutrality charged or with different baths, each mouth Donnan-equilibrated with its own bath (TMS); Hall access), `series_conductance` (closed-form check), `debye_length`, `blocking_mechanisms` (steric only) |
...
| `selectivity.py` | Vais 2010 through the pore: `ions(lumen, cytosol)` (asymmetric baths; impermeant NMDG⁺ left out), `ghk_ratio` (their Eq. 1) / `ghk_reversal` (its inverse), `reversal_potential` (root of the pore current), `selectivity` → `Selectivity` (P_Cl:P_K, P_Ca:P_K), `calcium_current` → `CalciumCurrent` (i_Ca at 0 mV, slope), `ghk_calcium_permeability` (Vais's GHK estimate), `selectivity_panel(st)` → `Reading`s (neutral / charged / paired / acidic only), `slow_nmdg_bound`, `published` |
