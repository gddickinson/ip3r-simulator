...
| `cli_ryr.py` | the RyR1 commands, `register(sub)`: `mutants`, `ryr-gating`, `sparks [--scan] [--cleft [--fit]]`, `spark-termination [--scan fit\|ki\|rate] [--fitted]`, `spark-mg [--scan] [--reading R] [--spontaneous]`, `ec [--scan] [--reading R] [--only S] [--trials N]` |
...
| `couplon.py` | the whole couplon: `CouplonParams` (C array + V model + V current), `step_protocol`, `v_trajectory` (one V channel through a voltage protocol, exact), `simulate_couplon` (V channels first, then the C array by Gillespie with V openings as scheduled events) → `CouplonTrace` (`flux()`) |
| `ec_release.py` | release under voltage clamp: `ensemble(v)` → `Ensemble`, `summarise` → `Summary` (peak, plateau, after repolarisation, flux peak/plateau), `configurations(reading)` (Stern / fitted / fitted + Mg²⁺ at the activation site / both sites), `with_gating`, `events`, `event_stats` → `EventStats` (single C events at `ec.spark_voltage`), `voltages` |
