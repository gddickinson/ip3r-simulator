...
| `cli_ryr.py` | the RyR1 commands, `register(sub)`: `mutants`, `ryr-gating`, `sparks [--scan]` |
...
| `sparks.py` | RyR1 cluster: `SparkParams`, `diffusion_coupling` (point source at the channel spacing), `simulate_sparks` (exact step tabulated per number open; returns a `PuffTrace`), `spark_couplings` |
...
| `puff_compare.py` | one ruler for all clusters: `MODELS` (the IP3R pair), `ALL_MODELS` (+ `SPARK` = `ryr1`), `spark_scan`, `MODEL_LABELS`, `params_for`, `simulate`, `event_sizes`, `recruitment` (Fano, open fraction, blips / multi / large ≥ half the cluster, rate), `coupling_effect(model=)`, `scan_couplings`, `coupling_scan` |
...
| `puffs_panel.py` | `PuffsPanel`: receptor (DYK / park-drive / RyR1 sparks, which ignore IP3), coupled vs uncoupled traces, event-size histogram, "Scan coupling (both receptors)"; `result` for the smoke test |
