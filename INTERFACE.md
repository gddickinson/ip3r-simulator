...
| `cli_ryr.py` | the RyR1 commands, `register(sub)`: `mutants`, `ryr-gating`, `sparks [--scan] [--cleft [--fit]]`, `spark-termination [--scan fit\|ki\|rate\|use\|recovery] [--bell] [--fitted]`, `spark-mg [--scan] [--reading R] [--spontaneous] [--two-site]`, `ec [--scan] [--reading R] [--only S] [--trials N] [--two-site] [--use] [--depletion [--pool-scan] [--large]]` |
...
| `ryr_use.py` | Laver & Lamb 1998's use-dependent (flux-driven) inactivation: `UseParams` (a `SternParams` plus a third gate entered **only from the conducting state**, so `s = a + 2i + 4u`, eight states, no detailed balance — `cycle_flux`; `available` = the ceiling it leaves, `tau_use`), `with_use`, `no_ca_gate` (the Ki-infinite control), `open_probability_no_ca_gate` (hand solution the tests hold the null space to), `fit_with_use` (Ka and Ki fitted *beside* the gate, so the measured bell is not counted twice; raises where no Ca²⁺ gate fits), `bell_panel` → `BellRow`, `tau_values` |
...
| `spark_termination.py` | what ends a cleft spark: `Termination` (sparks, median duration, `unterminated`, inactivated at end, open fraction), `measure(sp)`, `refit` (Stern vs fitted to Murayama 25/37 °C), `ki_scan`, `rate_scan(base)` (Ki fixed, rates scaled), `use_scan` (the use gate at every speed the bell permits, Ca²⁺ gate refitted at each), `recovery_scan` (its unsourced recovery rate, the round's load-bearing constant), `no_inactivation` (neither gate: the induction-decay control) |
