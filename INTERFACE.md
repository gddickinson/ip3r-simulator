...
| `docs/SCIENCE_PERM.md` | the pore permeation model: K⁺ conductance of each state, wall charge, salt bridges, and selectivity / i_Ca under Vais 2010's protocols |
...
| `cli_perm.py` | `register(sub)`: `selectivity [PDB] [--closure donnan\|radial]` (Vais 2010's P_Cl:P_K, P_Ca:P_K, i_Ca on every wall reading) |
...
| `selectivity.py` | Vais 2010 through the pore: `ions(lumen, cytosol)` (asymmetric baths; impermeant NMDG⁺ left out), `ghk_ratio` (their Eq. 1) / `ghk_reversal` (its inverse), `reversal_potential` (root of the pore current), `selectivity` → `Selectivity` (P_Cl:P_K, P_Ca:P_K), `calcium_current` → `CalciumCurrent` (i_Ca at 0 mV, slope), `ghk_calcium_permeability` (Vais's GHK estimate), `selectivity_panel(st, closure=)` → `Reading`s (neutral / charged / paired / acidic only), `slow_nmdg_bound`, `published` |
